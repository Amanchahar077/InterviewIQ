...
function Pricing() {
  ...
  const handlePayment = async (plan) => {
    try {
      ...
      const [configResult, orderResult] = await Promise.all([
        axios.get(ServerUrl + "/api/payment/config"),
      ...
      const razorpayKey = configResult.data.keyId;
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            ...
            console.log(error)
            setErrorMessage(error.response?.data?.message || "Payment verification failed. Please contact support if money was deducted.")
          } finally {
            setLoadingPlan(null);
          }
        },
      ...
      console.log(error)
      setErrorMessage(error.response?.data?.message || error.message || "Unable to start payment. Please try again.")
      setLoadingPlan(null);
    }
  }
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50 py-16 px-6'>
      ...
      {errorMessage && (
        <div className='max-w-3xl mx-auto mb-8 rounded-xl border border-red-200 bg-red-50 px-5 py-4 text-center text-sm font-medium text-red-700'>
          {errorMessage}
        </div>
      )}
