...
function Step2Interview({ interviewData, onFinish }) {
  ...
  const speakText = useCallback((text, { restartMic = true } = {}) => {
    ...
    const runIntro = async () => {
      if (isIntroPhase && !introSpokenRef.current) {
        ...
        if (currentIndex === questions.length - 1) {
          await speakText("Alright, this one might be a bit more challenging.", { restartMic: false });
        }
  ...
  }, [currentIndex, currentQuestion, isIntroPhase, isMicOn, questions.length, selectedVoice, speakText, startMic, userName])
  ...
  const repeatQuestion = async () => {
    if (!currentQuestion || isIntroPhase || isAIPlaying) return;
    await speakText(currentQuestion.question);
  }
  ...
  const submitAnswer = useCallback(async () => {
    ...
    try {
      ...
      await speakText(result.data.feedback, { restartMic: false })
    } catch (error) {
      console.log(error)
    } finally {
  ...
  const finishInterview = useCallback(async () => {
   ...
   useEffect(() => {
    ...
    if (timeLeft !== 0 || isSubmitting || feedback) return;
  ...
  }, [currentQuestion, feedback, isIntroPhase, isSubmitting, submitAnswer, timeLeft]);
    ...
    <div className='min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6'>
      <div className='w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
         ...
         {!feedback ? ( <div className='flex items-center gap-3 sm:gap-4 mt-6'>
            <motion.button
              onClick={toggleMic}
              whileTap={{ scale: 0.9 }}
              disabled={isAIPlaying || !isSpeechSupported}
              title={!isSpeechSupported ? "Voice input is not supported in this browser" : isMicOn ? "Turn microphone off" : "Turn microphone on"}
            ...
            <motion.button
              ...
              className='w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center rounded-full bg-white text-emerald-700 border border-emerald-200 shadow-md disabled:opacity-50'>
              <FaRedo size={18} />
            </motion.button>
